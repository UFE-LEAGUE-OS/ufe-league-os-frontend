    ...
    <div style={{ background: '#00030D', minHeight: '100vh', fontFamily: 'var(--font-body)', color: '#fff' }}>
      ...
      <main className="support-main" style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 56px' }}>
        ...
        <section style={{ marginBottom: 48 }}>
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: '1.3rem', marginBottom: 20 }}>HELP TOPICS</h2>
        ...
        {/* FAQ */}
        <section style={{ marginBottom: 48 }}>
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: '1.3rem', marginBottom: 20 }}>FREQUENTLY ASKED QUESTIONS</h2>
          <div className="support-faq" style={{ display: 'grid', gap: 12 }}>
            {faqItems.map((item, i) => (
              <details key={i} style={{ background: '#12131F', borderRadius: 8, padding: '16px 20px', border: '1px solid #1F2937', cursor: 'pointer', transition: 'border-color 0.2s' }}
                ...
                <summary style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: '0.92rem', color: '#fff', outline: 'none' }}>
                  {item.q}
                </summary>
        ...
        {/* Contact */}
        <section>
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: '1.3rem', marginBottom: 20 }}>CONTACT US</h2>
          <div className="support-contact-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {contactMethods.map((method) => (
              <div key={method.title} style={{ background: '#12131F', borderRadius: 12, padding: 24, border: '1px solid #1F2937', textAlign: 'center' }}>
                <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1rem', fontWeight: 600, marginBottom: 8, color: '#F97316' }}>{method.title}</h3>
          ...
          <div style={{ marginTop: 24, padding: 24, background: '#12131F', borderRadius: 12, border: '1px solid #1F2937' }}>
            <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1rem', fontWeight: 600, marginBottom: 12, color: '#fff' }}>Send us a message</h3>
            <div style={{ display: 'grid', gap: 12 }}>
              <div className="support-form-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <input type="text" placeholder="Your Name" style={{ padding: '12px 16px', borderRadius: 8, border: '1px solid #1F2937', background: 'rgba(255,255,255,0.04)', color: '#fff', fontFamily: 'var(--font-body)', fontSize: '0.85rem', outline: 'none' }} />
                <input type="email" placeholder="Your Email" style={{ padding: '12px 16px', borderRadius: 8, border: '1px solid #1F2937', background: 'rgba(255,255,255,0.04)', color: '#fff', fontFamily: 'var(--font-body)', fontSize: '0.85rem', outline: 'none' }} />
              </div>
              <textarea placeholder="How can we help?" rows={4} style={{ padding: '12px 16px', borderRadius: 8, border: '1px solid #1F2937', background: 'rgba(255,255,255,0.04)', color: '#fff', fontFamily: 'var(--font-body)', fontSize: '0.85rem', outline: 'none', resize: 'vertical' }} />
        ...
        {/* Quick Links */}
        <section style={{ marginBottom: 48 }}>
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, fontSize: '1.3rem', marginBottom: 20 }}>QUICK LINKS</h2>
            ...
            {quickLinks.map((link) => (
              <Link key={link.title} to={link.link} style={{ textDecoration: 'none', color: 'inherit' }}>
                <div style={{ background: '#12131F', borderRadius: 12, padding: 20, border: '1px solid #1F2937', transition: 'all 0.2s', cursor: 'pointer', height: '100%', textAlign: 'center' }}
                     ...
                     onMouseLeave={e => { e.currentTarget.style.borderColor = '#1F2937'; e.currentTarget.style.background = '#12131F'; }}>
                  <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '0.95rem', fontWeight: 600, marginBottom: 6, color: '#F97316' }}>{link.title}</h3>
        ...
        <section className="support-legal-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <Link to="/support" style={{ textDecoration: 'none', color: 'inherit' }}>
            <div style={{ background: '#12131F', borderRadius: 12, padding: 24, border: '1px solid #1F2937', transition: 'border-color 0.2s' }}
                 onMouseEnter={e => (e.currentTarget.style.borderColor = '#8135FA')}
                 onMouseLeave={e => (e.currentTarget.style.borderColor = '#1F2937')}>
              <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1rem', fontWeight: 600, marginBottom: 8, color: '#fff' }}>Terms of Service</h3>
          ...
          <Link to="/support" style={{ textDecoration: 'none', color: 'inherit' }}>
            <div style={{ background: '#12131F', borderRadius: 12, padding: 24, border: '1px solid #1F2937', transition: 'border-color 0.2s' }}
                 onMouseEnter={e => (e.currentTarget.style.borderColor = '#8135FA')}
                 onMouseLeave={e => (e.currentTarget.style.borderColor = '#1F2937')}>
              <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1rem', fontWeight: 600, marginBottom: 8, color: '#fff' }}>Privacy Policy</h3>
