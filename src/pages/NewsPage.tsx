...
export default function NewsSection() {
    ...
    useEffect(() => {
        axiosInstance.get("/accounts/feed/?unread_only=true")
            .then(res => {
                const results = Array.isArray(res.data?.results) ? res.data.results : [];
                setNotifications(results.map((item: Record<string, unknown>) => ({
                    id: Number(item.id),
                    message: String(item.title ?? item.message ?? item.content ?? "New update"),
